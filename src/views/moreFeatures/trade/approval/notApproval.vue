        ...
        <div v-show="steps==1">
            <el-form :model="formData" ref="formData" :rules="rules" class="mt20">
                <el-row>
                    ...
                    <el-col :span="14">
                        <el-form-item label="提交时间:" prop="transDate">
                            <el-date-picker
            ...
            <div class='mt20'>
                <el-card class='mt20' v-for='(item,index) in depositList' :key='index'>
                    ...
                    <div style='display:flex;margin-bottom:-40px;color:#999999;font-size:14px;'>
                        <span>提交时间：2020-06-31  12:09:12</span>
                        <span style='margin-left:23px'>下一级审批人：张三</span>
                        <span style='margin-left:36px'>审批截止日期：2021-06-30</span>
                    </div>
                    <div class='tar'>
                      <el-button size='small' @click='goDetail(index,item)'>审批详情</el-button>
                      <el-button size='small' v-show="item.state==1">撤销</el-button>
                    </div>
...
export default {
    ...
    methods: {
        ...
        goDetail(index,item){
            console.log(index)
            console.log(item)
            window.sessionStorage.setItem('notvalue', JSON.stringify(item));
            this.$router.push('/more/trade/approval/rest/not/notDetail')
        },
