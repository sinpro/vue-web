<template>
  <div class="fdDepositDraw">
    ...
    <title-bar v-if="step!=2" title="定期存款信息"></title-bar>
    ...
    <el-form v-if="step==0" :model="ruleForm" :rules="rules" ref="ruleForm" :inline="true" label-width="312px"
      ...
      <el-form-item label="转入账号:" prop="account">
        <el-input v-model="ruleForm.account" @input="changeMoney" placeholder="请输入" :disabled="true">
        </el-input>
      </el-form-item>
      <el-form-item label="支取金额:" prop="moneyArea1">
        <el-input v-model="ruleForm.moneyArea1" class="moneyArea" @input="changeMoney" placeholder="请输入" maxlength="24"
          :disabled="tips==2">
        </el-input>
        <el-button v-if="tips!=2" class="ml15" type="text">全部金额</el-button>
        <p v-if="tips==1" class="tipsClass f14 lh14 ctips">留存金额小于最低起存金额，请全额支取</p>
        <p v-if="tips==2" class="tipsClass f14 lh14 ctips">已提前支取{{tipsNum}}次，本次仅可全部支取</p>
        <p v-if="tips==2" class="tipsClass f14 lh14 cp1">壹拾万元整</p>
        <p v-if="tips==2" class="tipsClass f14 lh14 cp1">预计利息：<span class="cy">100.00</span></p>
      </el-form-item>
    </el-form>
    ...
    <div v-if="step!=2" class="wPro mtb20">
      <p>温馨提示</p>
      <p>1.我们；</p>
      <p>2.我们；</p>
      <p>3.我们；</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      goOver() {
        this.$router.push("/investmentFinancing/DepositBusiness/fixedDeposit/fdDeposit/index")
