...
export default {
   ...
   * @returns Promise
   */
  request(vm, method, url, params = {}, target = baseConfig.BASE_PROJECT.PRO_PROJECT/** 请不要更改此处，要更改内容，更改baseConfig即可 */,name='nu') {  // 默认为服务器的代理接口 target = '/eob-usercenter'
    let data
    if (name=='nu') {
      var newParams = {};
    ...
    return new Promise(function (resolve, reject) {
      ...
      axios.request(axiosConfig).then(function (response) {
        ...
        if (response.data.code == "BLEC0001") {
          ...
          vm.$Modal.warning({
            title: "提醒",
            content: "尊敬的用户，检测到您的账号长时间无操作，或在其他浏览器上登录，请您重新登录。"
          });
          vm.$router.push({ path: "/login" });
          ...
          // resolve(response);
          vm.$Message.error({
            content: response.data.body.errorMsg
          });
        }
