 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
            @selection-change  多选模式下 选中项变化时触发
                ...
                <tr>
                    <th v-for="(column,index) in cloneColumns" :key="index">
                        ...
                        <label v-else>
                            ...
                            <span class="el-table-sort" v-if="column.sortable">
                                <i class="el-icon-caret-top" :class="{on: column._sortType === 'asc'}" @click.native="handleSort(index, 'asc')" />
                                <i class="el-icon-caret-bottom" :class="{on: column._sortType === 'desc'}" @click.native="handleSort(index, 'desc')" />
                            </span>
                        </label>
                    </th>
                </tr>
                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'">
                            <el-button size="mini" :type="action.type" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</el-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0" :class="{'el-icon-circle-plus-outline':!item.expanded,'el-icon-remove-outline':item.expanded }"></i>
